...
const Autojoin =() =>{
    window.location.href="/public/autojoin.html";
}

const breakoutroom =() =>{
    $('#roomPromptModal').modal('show');
    
}

const loadfeatures =() =>{
    if (subroom=="yes"){
        var breakoutroomicon=document.getElementById("breakoutroom");
        breakoutroomicon.style.display="None";
        document.getElementById("leaveRoom").textContent="Leave Room";
        document.getElementById('notStartedMessage').style.display = 'None';
...
const joinRoom =(room) =>{
    ...
    if (room=="room1"){
        const room1Id = roomId + "room1";
        
        window.location.href=`/join?meeting_id=${room1Id}&name=${myname}&room='yes'&mainroom=${roomId}&startTime=${currentHour}:${currentMinute}&endTime=${endTime}`;
    }
        

    else if (room=='room2') {
        const room2Id = roomId + "room2";
        window.location.href=`/join?meeting_id=${room2Id}&name=${myname}&room='yes'&mainroom=${roomId}&startTime=${currentHour}:${currentMinute}&endTime=${endTime}`;
    } 
    
    else {
        const room3Id = roomId + "room3";
        window.location.href=`/join?meeting_id=${room3Id}&name=${myname}&room='yes'&mainroom=${roomId}&startTime=${currentHour}:${currentMinute}&endTime=${endTime}`;
    }
}
...
const leaveroom =() =>{
    if (subroom=="yes"){
        const oldroom=roomId.substring(0, roomId.length - 5);
        window.location.href=`/join/${oldroom}?name=${myname}`;
    }
    else{
